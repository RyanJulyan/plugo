...
import subprocess
import sys
from pkg_resources import Requirement as PkgRequirement, DistributionNotFound, VersionConflict, get_distribution


def load_plugins(plugin_directory, config_path, logger=None, **kwargs):
    if not logger:
        # Create a logger
    ...
    if not os.path.exists(plugin_directory) or not os.path.isdir(plugin_directory):
        logger.error(f"Plugin directory '{plugin_directory}' does not exist or is not accessible.")
        return  # Stop execution if the directory does not exist
    
    # Check if the plugin configuration file exists
    if not os.path.exists(config_path):
        logger.error(f"Plugin configuration file '{config_path}' does not exist or is not accessible.")
    ...
    if env_plugins:
        env_plugin_list = [plugin.strip() for plugin in env_plugins.split(',') if plugin.strip()]
        enabled_plugins.update(env_plugin_list)
        disabled_plugins.difference_update(env_plugin_list)  # Remove from disabled if enabled via env var
    ...
    for plugin_name in os.listdir(plugin_directory):
                    ...
                    if line and not line.startswith('#'):
                        try:
                            ...
                            try:
                                ...
                                logger.info(f"Requirement '{line}' already satisfied for plugin '{plugin_name}'.")
                            except (DistributionNotFound, VersionConflict) as e:
                                logger.info(f"Installing requirement '{line}' for plugin '{plugin_name}'.")
                                # Install the requirement using pip
                                subprocess.check_call([sys.executable, "-m", "pip", "install", line])
                        except Exception as e:
                            logger.error(f"Error processing requirement '{line}' in plugin '{plugin_name}': {e}")
        else:
        ...
        if not os.path.exists(metadata_path) or not os.path.exists(plugin_main):
            logger.warning(f"Plugin `{plugin_name}` is missing required files: `metadata.json` or `plugin.py`.")
            continue
        ...
        try:
            ...
            logger.error(f"Error decoding JSON from metadata file for plugin '{plugin_name}': {e}")
            continue
    ...
    def visit(plugin_name):
        if plugin_name in temp_marks:
            ...
            raise Exception(f"Circular dependency detected involving plugin '{plugin_name}'.")
        if plugin_name not in visited:
            ...
            if not plugin:
                ...
                raise Exception(f"Plugin '{plugin_name}' not found in plugin directory.")
            # Check if the plugin is explicitly disabled
            if plugin_name in disabled_plugins:
                logger.error(f"Plugin '{plugin_name}' is required but is explicitly disabled.")
                raise Exception(f"Plugin '{plugin_name}' is required but is explicitly disabled.")
    ...
    try:
        ...
        for plugin_name in list(enabled_plugins):  # Use list() to create a static list
            if plugin_name not in plugin_info:
                logger.error(f"Plugin '{plugin_name}' specified as enabled but not found in plugin directory.")
                continue
            if plugin_name not in visited:
                visit(plugin_name)
    except Exception as e:
    ...
    for plugin_name in loading_order:
        ...
        try:
            spec = importlib.util.spec_from_file_location(f"{plugin_name}.plugin", plugin_main)
            ...
            if hasattr(plugin_module, 'init_plugin'):
                ...
                logger.warning(f"Plugin `{plugin_name}` does not have an 'init_plugin' function.")
        except Exception as e:
    ...
    for plugin_name in not_loaded_plugins:
        if plugin_name in disabled_plugins:
            ...
            logger.warning(f"Plugin `{plugin_name}` was not loaded for unknown reasons.")
    ...
    for plugin_name in disabled_plugins:
        if plugin_name not in all_plugins_in_directory:
            logger.warning(f"Plugin '{plugin_name}' is specified as disabled but not found in plugin directory.")
        else:
